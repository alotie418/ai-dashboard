...
> **状态订正（2026-08-10）**：括注「当前环境缺失，未运行签名构建」写于本文件落库时（2026-07-15 08:41），**当时为真**——两张证书的签发时间是同日 09:21 / 09:22，profile 是 09:42。此后**签名构建确实跑过**：工作区 `release/mas-arm64/` 留有同日 09:51–10:00 产出的 `SoloLedger.app`（含 `_CodeSignature/` 与内嵌 profile）与 `SoloLedger-1.0.0-arm64.pkg`，其 `CFBundleVersion` = `1.0.1`。**但该产物从未上传 ASC**（见 [`MAS_SUBMISSION.md`](MAS_SUBMISSION.md) 状态节），所以本文件整篇描述的重新提审**未曾发生**，它是一份**已备好而未执行**的整改方案。下表因此仍是有效的前置清单，只是三项今天都已就位。
