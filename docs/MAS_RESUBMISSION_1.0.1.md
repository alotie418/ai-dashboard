...
> # 🧊 本方案已永久作废（2026-08-15 / 2c-10），文档保留作历史记录
>
> **不要执行本文的任何一步。** 它描述的是一次**已备好而从未执行**的 Electron MAS 重新提审；随
> Electron MAS 线冻结（D6）与原生取代式上架（D1），这次重提不会再发生。
>
> 作废的机制，**分两层说清楚，别把本仓纪律写成 Apple 的规则**：
>
> * **Apple 侧**的规则只是同一 version train 内构建号不冲突；
> * **本仓采用的是更严的保守下限**——构建号必须高于该 bundle id 下**一切**已被接受的构建
>   （`AppVersionGuardTests` 里的 `appStoreConnectHighWaterMark = "1.0.0"`）。原生版按 D2 把构建号
>   钉成纯整数、从 `2` 起，按分段比较 `1.0.1 < 2`，所以原生一旦上传，本文这份 `1.0.1` 在这条自订
>   下限下就再也排不上号。**这是本仓自己的纪律造成的后果，不是 Apple 的硬性拒绝。**
>
> 用户 2026-08-11 已裁定接受这一跨产线后果（2c-3）。**本文以下内容一字未改**，因为它记录的是当时
> 真实做过的整改分析（五条 Guideline 的逐条答复仍有参考价值），只是不再是可执行方案。冻结告示见
> [`MAS_SUBMISSION.md`](MAS_SUBMISSION.md) 文首。
