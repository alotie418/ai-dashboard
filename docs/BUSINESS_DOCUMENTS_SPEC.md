...
* **对账单是唯一的模板变体（第九次裁定）**：它的明细表出**四列**——描述 / 日期 / 金额 / 税额——**不出税率**，且**日期是独立的一列**，取自 `business_document_items.ref_date`。这与 Q2-b 在屏幕侧定的是同一条口径：Electron 把日期揉进描述串（`salesToRow` 的前缀 `` `${r.date} ${...}` ``），原生把它拿掉了，所以在产物里照抄六列会把日期整个丢掉。**其余四种类型仍是六列。** 实测：Electron 对一张原生形状的对账单出的是六列、其中三列全空、日期在描述格里。
