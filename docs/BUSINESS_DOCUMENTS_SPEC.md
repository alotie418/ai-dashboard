...
* **泄漏点与 D-6 接缝的完整清单（第十二次裁定补齐）——五处，不是三处**：
  ...
  前三处只改内存里的草稿状态，Core 完全不知道它们发生过；后两处有回传值。**「取消」此前只出现在
  Electron 那三条删除路径的括号里，没有进过本清单**，故一并补上。
* **接缝是单点的**：App 侧消费那两个回传值的地方各恰一处，都在页面模型里，都只是把值丢掉。
  **原子化轮不接**：它落地了条件写与安全删除原语（`AttachmentDeletion`），两个 `_ = try …` 丢弃点
  一字未改，App 目标对该原语的命名数为 **0**。
