...
* **接缝是单点的**：App 侧消费那两个回传值的地方各恰一处，都在页面模型里，都只是把值丢掉。
  **原子化轮不接**：它落地了条件写与安全删除原语（`AttachmentDeletion`），两个 `_ = try …` 丢弃点
  一字未改，App 目标对该原语的**代码**命名数为 **0**（守门读的是剥注释后的源码；本轮订正过的那两处注释
  提到了它的名字，那是散文不是调用）。
...
**本轮不假装 E 已解决。** 它不属于 B 系列（B 系列记的是与 Electron 之间**已定**的有意差异，而这是一个
**未决**的缺口），也不新开 A 编号（A 系列记的是 Electron 的已知形态）。它就登记在这里，作为存储原子化的
残余，并且是 **D-6 接删除前的强制前置裁定**：在 D-6 把上面那五处接缝中的任何一处接上删除之前，用户必须先
在下面两条里裁定一条——
...
**没有为 E 写任何测试。** 一条永远为绿的测试声称一个未修的缺口已经关上，比缺口本身更坏；
`AttachmentDeletionTests` 里唯一与 E 有关的断言，是断言**本文件确实这样登记了它**。

#### 第二处残余：`unlink` 自身的相邻系统调用窗口（#494 复核提出，**待裁定**）
...
不是新引入的缺陷；③本轮 App 侧零调用，实害当前不可达。

**处置：登记并停下要裁定，与竞态 E 并列成为 D-6 接删除前必须先答的第二个问题。** 三个候选方向 ——
...
| **D-6 激活** | 侧栏可达 + 接上附件删除接缝（**两个并列强制前置，都必须先裁定，见 §5**：**a. 竞态 E**——文件删掉后那个相对名被重新认领；**b. `fstatat`→`unlinkat` 相邻系统调用窗口**——同 UID 进程在这两个相邻系统调用之间换掉同名条目，被删的是替身。两条各自独立，**答一条不解另一条**）+ 收尾 | 侧栏入口可达；**`SidebarSection`（`AppModel.swift`）新增一例、`RootView` 加分支、`InventoryView` 的「不是本页的五个分区」注释与 `AppModel` 的「六个分区之一」注释同轮改**（第七次裁定自 D-4 移入；实测届时另有 8 处 `SidebarSection.allCases` 有序断言与 `DocumentCopyTests` 的 `SidebarSectionProbe` 两条要同批翻转，开工前须重跑扫描）；`LegacyLedgerProbe` 的 `otherRecordTables` 移除 `business_documents` 与 `business_document_items` 两项；`legacy.other.message` 六语改写（现文说「本 App 目前只显示流水」，激活后变假）；`docs/SWIFTUI_FEATURE_GAP.md` §3 该行改状态 | +10~20 | `products` 已有同形先例，其理由写在 `AppModel.swift` 对 `otherRecordTables` 的注释里 |
...
**两处开工闸门（第十二次裁定新增）。** 上面那个「无」说的是**测绘九问**——本章的口径没有留白，Q1–Q9 全部已裁定，这一点未变。但存储原子化轮新产生了**两个必须先裁定才能继续**的技术缺口。它们都不是九问里的哪一问，所以不在上表；两条都是 **D-6 接删除前的强制前置裁定**，且**彼此独立——答了一条不解另一条**：

| 闸门 | 缺口 | 写在哪 |
| --- | --- | --- |
| **a. 竞态 E** | 附件文件删掉之后，那个相对名就空出来了，陈旧或不协作的写入者可以**重新认领同一路径** | **§5 的「竞态 E · 存储原子化的残余」一节**（三个候选方向列在那里） |
| **b. `fstatat`→`unlinkat` 相邻系统调用窗口** | `unlinkIfStillBound` 先 `fstatat` 校验身份、再 `unlinkat` **按名字**删；同 UID 进程在这两个相邻系统调用之间换掉同名条目，被删的是替身而不是绑定的 inode | **§5 的「第二处残余：`unlink` 自身的相邻系统调用窗口」一节**（三个候选方向列在那里） |

**D-6 开工前必须先读那两节**，不能只看本节的「无」就认为没有阻塞。
