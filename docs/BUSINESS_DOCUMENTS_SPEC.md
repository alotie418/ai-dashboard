...
| A9 | **`update` 的状态读在事务之外**：先无事务地读出 `status`，在 JS 里判完草稿规则与状态机，随后才开事务写。读与写之间存在窗口 | `electron/handlers/documents.js` 的 `update`：`const existing = db.prepare('SELECT id, status …').get(id)` 在函数顶部，`db.transaction(…)` 只包住 `UPDATE`／明细重写 | ~~照搬~~ **已被第十二次裁定取代**：原生改条件写 + 受影响行数校验。**Electron 侧的形态与本行记录的事实一字不改**，编号保留、不退役、不复用；原生与它的差异另立 **B11** |
| A10 | **`remove` 全程无事务**：读 `status` 与附件路径 → 判 `issued` → `DELETE` → best-effort 删附件，四步各自独立 | 同文件 `remove`：整个函数内**没有** `db.transaction` | ~~照搬~~ **已被第十二次裁定取代**：同上，另见 B11 |
| A11 | **`updateTaxInvoice` 全程无事务**：读 `status` 与既有路径 → 判 `void` → 查附件是否被别的单据占用 → `UPDATE` → best-effort 删旧副本，五步各自独立 | 同文件 `updateTaxInvoice`：整个函数内**没有** `db.transaction`；占用查询是独立的 `SELECT 1 … AND id != ?` | ~~照搬~~ **已被第十二次裁定取代**：同上，另见 B11 |
...
1. **Electron 同形，逐字段对得上。** 三处窗口不是原生引入的：`update` 的状态读在事务外（better-sqlite3 的 `db.transaction` 默认发的是 DEFERRED `BEGIN`，且它只包住写）；`remove` 与 `updateTaxInvoice` **零事务**。Q9 的验收线是「同输入下结果与 Electron 逐字节相等」，原生在这三处是忠实照搬。**（这一句写于第五次裁定，描述的是当时的实现；第十二次裁定已把三处改成条件写，原生不再照搬，见 B11。本行保留原文，因为它记录的是当时的依据。）**
...
**「附件删除」这一触发器有一条明写的例外（第十二次裁定）**：`BackupRestore.clearActiveSlot` 会移除
`config.activeAttachmentsDir` 整个目录，但它**不是**本条款所指的路径。理由是它的形状不同，而不是它的
后果轻：它只在**恢复备份**这条路上被调用，调用前活动 store 已经 `close()`、`store = nil`、`ready = false`，
之后由加固导入链整个重建 DB 与附件；它删的是**整槽**，不是某一个仍可能被别的行引用的副本，也不与任何
check-then-act 判据同处一个窗口。本条款要防的是**在线的、逐附件的删除**——先查「没被引用」再 `unlink`，
两步之间可以被第二个连接插进来。`clearActiveSlot` 不是那个形状，所以它的存在**没有**在原子化轮之前触发
升级条款；这一句写在这里，是因为不写清楚的话，下一个读到条款的人会合理地认为条款早就被触发过了。

**第十二次裁定同时明确批准了 A9–A11 的条件写这一处有意偏离。** §1 的「除 Q2 外不接受发明」与 Q9 的
「同输入下与 Electron 逐字节相等」都还成立；条件写不是发明出来的新口径，而是**用户在被明确告知它是有意
偏离之后所作的裁定**，并已按 A/B 两系列的分工登记：Electron 侧的形态留在 A9–A11，原生侧的差异新立 B11。
...
* **泄漏点与 D-6 接缝的完整清单（第十二次裁定补齐）——五处，不是三处**：
  1. **重选**（已有关联时再选一个文件，旧副本落单）；
  2. **移除**（清掉未保存的关联，刚复制进来的副本落单）；
  3. **取消**（关掉关联面板而不保存，同上）；
  4. **保存时替换/清除**（`updateTaxInvoice` 写新路径或写空，旧副本落单——这一处会**回传**孤儿路径）；
  5. **删单据**（`deleteBusinessDocument`，同样**回传**孤儿路径）。

  前三处只改内存里的草稿状态，Core 完全不知道它们发生过；后两处有回传值。**此前本章从未把这五处写成
  一份清单**：本节上方那句「重选、移除、取消一律只改状态」点了前三处、`discardDocAttachment` 的括号里
  点了同样三处，而带回传值的那两处散在别处，没有一处把五个站点并排列出来。补的是这份清单本身。
* **接缝是单点的**：App 侧消费那两个回传值的地方各恰一处，都在页面模型里。
  **原子化轮不接**：它落地了条件写与安全删除原语（`AttachmentDeletion`），两个 `_ = try …` 丢弃点
  一字未改，App 目标对该原语的**代码**命名数为 **0**（守门读的是剥注释后的源码；那一轮订正过的两处注释
  提到了它的名字，那是散文不是调用）。
* **D-6 已把五处全部接上**（第十三次裁定回答了两道前置之后）。上面那句「命名数为 **0**」与两个
  `_ = try …` 丢弃点**都是存储原子化轮的实测，对今天为假**——原文保留，因为它记录的是当时的验收线。
  D-6 之后：五处统一调用 `AttachmentDeletion.deleteIfUnreferenced`，前三处（重选／移除／取消）传的是
  草稿刚失去所有权的那条相对路径、由原语自己扫两根权威引用列决定删不删，后两处吃 `updateTaxInvoice` /
  `deleteBusinessDocument` 从**真正被写／被删的那一行**回传的孤儿路径。**五处一律在数据库事务之外调用**
  （原语自开 `BEGIN IMMEDIATE`，嵌套会 fail-closed 成 `.unavailable`），**清理失败一律不反报**已经成功的
  保存或删除。
...
**B11（第十二次裁定，存储原子化轮）在写入面上**，是本系列第一条**不是**由镜像取舍产生、而是由用户
明确批准的偏离产生的条目：A9–A11 三处 check-then-act 改成条件写，Electron 那一侧一个字不改。它与
B3–B10 的另一个区别是**方向**——其余各条要么是本仓给不出同一个东西，要么是原生有意收窄了输入；这一条
是原生在**同一个输入上给出更严的结果**（拒绝写），而对面会写。
...
| B11 | **三处生命周期写是条件写，Electron 是无条件写**：原生 `update` / `deleteBusinessDocument` / `updateTaxInvoice` 把已判断的事实并进最终写语句的谓词，并校验受影响行数——`update` 的谓词钉的是**规则**（改字段/明细 ⇒ `status IN ('draft')`；改状态到 `W` ⇒ `status IN (<能到达 W 的那些状态>)`），`remove` 是 `status != 'issued'` 且孤儿路径由 `RETURNING` 从**真正被删掉的那一行**取出，`updateTaxInvoice` 是 `status != 'void'` + 占用的 `NOT EXISTS` + 「这一行仍持有本次请求以为要替换的那个副本」。行数为 0 时重读事实、按既有顺序给出同一个稳定错误码——**除两处例外**：①「要的状态已经成立」（只改状态、而那个状态现在正是行上的状态）是**静默成功**，因为那正是前置检查自己对同一组事实给出的答案（比较在校验之前，既不校验也不写）；②别的写入者把被判断的那一行搬走了（走了状态机没有的边，或改掉了本次要替换的附件），六个稳定码没有一个说的是这件事，故走非 `BusinessDocumentError`。Electron 三处都是读完就无条件写 | 原生 `DocumentStore.swift` 的 `applyDocumentEdit` / `applyDocumentDelete` / `applyTaxInvoiceEdit`；反向证明在 `DocumentLifecycleTests`「第十二次裁定」一节；Electron 侧形态见 A9–A11 | **有意偏离，第十二次裁定明确批准。** 单进程时两侧结果相同——差异只在有第二个连接插进读与写之间时才显形，且方向是原生更严：它拒绝写，Electron 会写。**一处无稳定错误码的残余**：`updateTaxInvoice` 的第三个谓词落空（别的写入者把这一行的附件改掉了）时，六个稳定码没有一个说的是这件事，故以非 `BusinessDocumentError` 离开，页面按它既有的通用「保存失败，请重试」呈现——**这是准确的，因为确实什么都没写**。不新增错误 case、不新增六语键、不新增文案 |
...
范围**三件事**。前两件绑在一起，是因为它们是同一类问题（判据写在宿主语言里，而不是写在写语句或结果码
里）；第三件由第七次裁定并入，它不是同一类问题，而是**同一条排期链上的前置条件**——D-6 要接删除，而接
删除会触发 §3 的升级条款，条款要求前两件先落地。

> **本段原写「两件」，由第十二次裁定外科式订正为「三件」。** 第三件早在第七次裁定就已写在下面的第 3 条
> 里，本段的开头句与 §6 拆轮表的「三件」当时没有跟着改，于是同一节里同时写着两件和三件。本次只改这处
> 计数与它的绑定理由，不重写这一节的历史。

1. **三处 check-then-act 收口**（§3 A9 / A10 / A11）：`update` / `deleteBusinessDocument` / `updateTaxInvoice` 改成把被判断的事实写进谓词的**条件写**并**校验受影响行数**，行数为 0 时给出与今天同一个稳定错误码。**不改 schema**——这是与「加唯一约束」那条路的分野。附件归属那一处同理：把「没被别人占用」并进写语句的谓词。

   > **谓词的形状由第十二次裁定订正。** 本条原写 `… WHERE id = ? AND status = ?`。实现时改成钉**规则**而不是钉**读到的值**：`status = <读到的>` 会拒绝一个对新状态仍然合法的请求（草稿要作废、期间被别人签发，那是 `issued → void`），而「合法但没写」没有稳定错误码可给。最终形状与三处各自的 0 行处置见 **B11**。
2. **`ProductCatalog.mapWriteFailure` 的 `(code 19)` 谓词修正**：D-2 实测坐实它在出货连接上一条也匹配不到——`activeExistingNoFollow` 带 `SQLITE_OPEN_EXRESCODE`，同一条唯一索引冲突报的是 `(code 2067)`（PK/NOT NULL/FK/CHECK 分别 1555/1299/787/275，**主码全是 19**）。改法与 D-2 的 `mappingConstraintToNumberExists` 同形：判主结果码。属库存页，D-2 不越界修，只留证据。
   ...
   > **本条的 `787` 由第十二次裁定订正。** D-2 当时是在别处量的 FK 码；`products` 上真正发生的那条外键是 `inventory_movements.product_id … ON DELETE RESTRICT`，而 **RESTRICT 是一个 action**，SQLite 用 `SQLITE_CONSTRAINT_TRIGGER` = **1811** 报它，不是 `SQLITE_CONSTRAINT_FOREIGNKEY` = 787。787 是 NO ACTION 引用的码，本轮在一个一次性库里实测出来只作对照。完整矩阵见 §9 第十二次裁定那一行。

3. **附件的安全删除**：把 D-4 登记的那条泄漏收掉，
   并把「没被别人占用」并进写语句的谓词（与第 1 件同形）。这一件是本轮**排期被提前**的直接原因：
   D-6 要接删除接缝，而接删除会触发 §3 的升级条款。

   > **本条的范围由第十二次裁定订正为两句话。** 原文写的是「重选 / 移除 / 替换 / 删单据时**真的删掉**不再被
   > 引用的副本」，那句话有两处不准：①**站点是五个不是四个**（漏了「取消」，完整清单见 §3 该节）；②**原子化
   > 轮不真的删**——它落地的是**原语**（`AttachmentDeletion`）与它的证明，App 目标对该原语的命名数为 0（剥注释后），
   > 两个 `_ = try …` 丢弃点一字未改。**接线仍然是 D-6 的**，且当时还多**两道并列前置**，都必须先裁定：
   > **竞态 E** 与 **`fstatat`→`unlinkat` 相邻系统调用窗口**（两节都在 §5，答一条不解另一条）。
   > **两道前置已由第十三次裁定分别回答，D-6 已按裁定接线**；两处残余各自仍在效，见 §5 与 §8。

三件都要走反向证明；第 1 件与第 3 件还要各配一个「只有它能杀掉」的用例（条件写被改回无条件写、
受影响行数校验被删、占用查询与删除之间的窗口）。

#### 竞态 E · 存储原子化的残余（第十二次裁定登记；第十三次裁定选定「无 schema 协调」）

第 3 件把**引用扫描与 `unlink` 之间**那个窗口（记作 **D**）关上了：两步同处一个 `BEGIN IMMEDIATE`，写锁
全程持有，第二个连接无法在中间提交一条新的引用。这一条有真实双连接实测，不是推导。

**关不上的是它后面那个窗口，记作 E。** 文件一旦 `unlink`，那个相对名就**空出来了**；任何不与本进程协作的
写入者——或者一个在删除之前就已经算好 `attachments/docs/<name>` 的陈旧写入者——都可以**重新认领同一路径**，
把一条指向「刚被删掉的那个名字」的引用写进库里。此后库里那条引用指向的，要么是不存在的文件，要么是别人
后来放在同名位置上的另一个文件。

**不假装 E 已解决。** 它不属于 B 系列（B 系列记的是与 Electron 之间**已定**的有意差异，而这是一个
**未闭合**的缺口），也不新开 A 编号（A 系列记的是 Electron 的已知形态）。它就登记在这里，作为存储原子化的
残余。它曾经是 **D-6 接删除前的强制前置裁定**，两个候选方向是——

1. **无 schema 协调**：只在协作写入者之间约定（例如名字一次性、不复用，或删除走一个统一入口），并**同时
   裁定拿外部写入者的残余怎么办**——因为这条路对不协作的写入者没有约束力，那部分残余会留下来；
2. **schema/所有权记录**：给附件一条能表达「这个名字归谁、是否已释放」的记录。这会动 schema，而 Q9 明禁
   加表加列（唯一例外是 D-1a 的 `currency`），所以它本身也需要一次单独裁定。

**第十三次裁定选定第 1 条：无 schema 协调。** 本章不增加附件所有权表、墓碑列或任何其它 schema 结构。
受支持的 App 写入者遵守四条约定：

* **名字一次性。** 每次选择附件都现场生成新文件名，不主动复用一个已经释放的相对路径。
* **创建保持 no-overwrite。** 候选名已存在时只能失败或换一个新名字，不得覆盖既有条目。
* **不缓存失去所有权的旧路径。** 一条已经不再属于本草稿的相对路径，不得在删除之后又被写回库里。
* **删除只走一个入口。** 五处接缝统一调用 `AttachmentDeletion.deleteIfUnreferenced`，不得各写一套删除逻辑。

**同一次裁定明确接受它留下的残余，不得写成「竞态 E 已关闭」。** 这条路对**不协作的外部写入者**与**陈旧写入者**
没有约束力：它们仍然可以在文件被删之后重新认领同一个相对名，把一条指向那个名字的引用写进库里。此后库里
那条引用指向的，要么是不存在的文件，要么是别人后来放在同名位置上的另一个文件。**这一部分残余是被接受的，
不是被消除的。**

> **自动升级条款（自动生效，不需要再裁定一次）**：一旦引入**受支持的多进程写入**、**云同步**、**监控文件夹**、
> **外部编辑器回写**，或**任何跨越删除缓存旧路径的正式入口**，本裁定自动失效，必须重新停下来裁定
> schema／所有权记录。

**没有为 E 写任何测试。** 一条永远为绿的测试声称一个未闭合的缺口已经关上，比缺口本身更坏；
`AttachmentDeletionTests` 里唯一与 E 有关的断言，是断言**本文件确实这样登记了它、并且确实选的是第 1 条**。

#### 第二处残余：`unlink` 自身的相邻系统调用窗口（#494 复核提出；第十三次裁定**接受并登记残余**）

`unlinkIfStillBound` 是 `fstatat` 然后 `unlinkat(parent.fd, name, 0)`，而 **`unlinkat` 删的是一个名字**。
同 UID 的另一个进程若在这两个相邻系统调用之间把同名条目换掉，被删的是它换进来的那个，不是本次绑定的
inode。**「替身不被删」这条性质因此只对「检查之前发生的替换」成立，对「落在这两个系统调用之间的替换」
不成立。** 原语里 `matchesChild` 那一次预检并不缩小这个窗口 —— 它是第二次 check-then-act，作用是把
`.movedUnderUs` **报出来**，不是拦住什么。

**Darwin 没有按 inode 删除的系统调用**，所以这一处不是把调用写法改一改就能关上的：改成「先改名成唯一名、
再删那个唯一名」只是把窗口挪个位置，还多出一条失败时留下孤儿条目的路。而第十二次裁定明确要求本原语
**复用** `unlinkIfStillBound`、不得另写变体，所以它的形状不由实现轮决定。

登记依据三条：①它与 `PreparedImportActivator` 文件头已登记的同 UID、point-in-time 残余（fingerprint→rename
的相邻窗口）是同一类，那里的结论也是「Darwin 上关不掉，检测是事后的、fail-closed 的，不是预防性的」；
②Electron 的 `safeDeleteAttachment` 是 `fs.rmSync` 按路径删，**完全没有身份校验**，所以原生这一侧严格更强，
不是新引入的缺陷；③（写于存储原子化轮）当时 App 侧零调用，实害不可达。

> **依据③已由 D-6 作废，保留原文以记录当时写下的东西。** D-6 把五处接缝接上了
> `AttachmentDeletion.deleteIfUnreferenced`，App 侧调用数不再是 0，所以「实害当前不可达」这句话对今天为假。
> 依据①与②不受影响，且第十三次裁定接受残余时是**在知道 D-6 就要接线**的前提下作出的——它接受的正是接线
> 之后的那个残余。

**原处置：登记并停下要裁定，与竞态 E 并列成为 D-6 接删除前必须先答的第二个问题。** 三个候选方向 ——
(a) 接受它，按 `PreparedImportActivator` 的先例写成已登记残余；(b) 换一种删除方案（例如改名后删，接受
它自己的窗口与孤儿条目）；(c) 只在一个本进程独占、外部写入者被排除的目录上做删除，把前提从「原子」换成
「独占」。

**第十三次裁定选定 (a)：接受并登记残余。** 保留现有的 `unlinkIfStillBound`，**不另写改名后删除的变体**，
**不把附件目录描述成获得了系统级独占**，并**明确认可**同 UID 进程在这两个相邻系统调用之间替换同名目录项
时，替身可能被删。

四条依据：①Darwin 没有按 inode 删除的系统调用；②改名后删除只把窗口挪个位置，并新增一条失败后留下孤儿
文件的路；③原生这一侧仍**严格强于** Electron 的 `safeDeleteAttachment`（`fs.rmSync` 按路径删，零身份
校验）；④支持范围内的单据写入由 App 的主执行序列协调——**但这不足以、也不得被说成防住敌对或不协作的同 UID
进程**。

**以下过度保证在本章任何地方都不得出现**：「替身永远不会被删除」「删除针对绑定 inode 原子完成」
「相邻系统调用窗口已经关闭」「目录已被系统级独占」。

> **自动升级条款（自动生效，不需要再裁定一次）**：若发现**受支持的 App 内部路径可以并发替换同名目录项**、
> **附件目录改为共享或同步目录**，或**产品需要承诺抵抗同 UID 写入者**，本裁定自动失效，必须**暂停删除接线**
> 并重新裁定。
...
| **D-6 激活** | 侧栏可达 + 接上附件删除接缝（**两个并列强制前置已由第十三次裁定分别回答，见 §5**：**a. 竞态 E** → 选定「无 schema 协调」，**外部/陈旧写入者的残余被接受而非消除**，带自动升级条款；**b. `fstatat`→`unlinkat` 相邻系统调用窗口** → 选定「接受并登记残余」，**明确认可替身可能被删**，带自动升级条款。两条各自独立，当初**答一条不解另一条**，现在两条都已答。**D-6 获准接线的条件即此：两条都已裁定，且接线只调用既有的 `AttachmentDeletion.deleteIfUnreferenced`、不改其 syscall 形状、不新增 schema／错误 case／六语键／用户可见文案**）+ 收尾 | 侧栏入口可达；**`SidebarSection`（`AppModel.swift`）新增一例、`RootView` 加分支、`InventoryView` 的「不是本页的五个分区」注释与 `AppModel` 的「六个分区之一」注释同轮改**（第七次裁定自 D-4 移入；实测届时另有 8 处 `SidebarSection.allCases` 有序断言与 `DocumentCopyTests` 的 `SidebarSectionProbe` 两条要同批翻转，开工前须重跑扫描）；`LegacyLedgerProbe` 的 `otherRecordTables` 移除 `business_documents` 与 `business_document_items` 两项；`legacy.other.message` 六语改写（现文说「本 App 目前只显示流水」，激活后变假）；`docs/SWIFTUI_FEATURE_GAP.md` §3 该行改状态 | +10~20 | `products` 已有同形先例，其理由写在 `AppModel.swift` 对 `otherRecordTables` 的注释里 |
...
**两处已经分别裁定的残余边界（第十二次裁定登记，第十三次裁定分别回答）。** 上面那个「无」说的是**测绘九问**——本章的口径没有留白，Q1–Q9 全部已裁定，这一点未变。存储原子化轮另外产生过**两个必须先裁定才能继续**的技术缺口。它们都不是九问里的哪一问，所以不在上表；两条曾经都是 **D-6 接删除前的强制前置裁定**，且**彼此独立——答了一条不解另一条**。**两条现在都已由第十三次裁定回答，D-6 的接线闸门因此打开；但两条各自留下的残余仍然在效，各带一条自动升级条款**：

| 闸门 | 缺口 | 第十三次裁定 | 被接受的残余 | 写在哪 |
| --- | --- | --- | --- | --- |
| **a. 竞态 E** | 附件文件删掉之后，那个相对名就空出来了，陈旧或不协作的写入者可以**重新认领同一路径** | **无 schema 协调**（两个候选方向里的第 1 条） | **外部写入者与陈旧写入者仍可重新认领旧名字。E 不得被写成「已关闭」。** 升级触发器：受支持的多进程写入／云同步／监控文件夹／外部编辑器回写／任何跨越删除缓存旧路径的正式入口 | **§5 的「竞态 E · 存储原子化的残余」一节**（**两个**候选方向列在那里） |
| **b. `fstatat`→`unlinkat` 相邻系统调用窗口** | `unlinkIfStillBound` 先 `fstatat` 校验身份、再 `unlinkat` **按名字**删；同 UID 进程在这两个相邻系统调用之间换掉同名条目，被删的是替身而不是绑定的 inode | **接受并登记残余**（三个候选方向里的 (a)） | **同 UID 进程在这两个相邻系统调用之间换进来的替身可能被删。不得声称窗口已关闭、删除对绑定 inode 原子完成、或目录已被系统级独占。** 升级触发器：受支持的 App 内部路径可并发替换同名目录项／附件目录改为共享或同步目录／产品需要承诺抵抗同 UID 写入者 | **§5 的「第二处残余：`unlink` 自身的相邻系统调用窗口」一节**（三个候选方向列在那里） |

**D-6 及其之后的任何轮次仍必须先读那两节**，不能只看本节的「无」就认为两处残余不存在——它们已被裁定，不等于已被消除。
...
### 2026-08-20 · 第十二次裁定（存储原子化轮：三处条件写、`ProductCatalog` 谓词、安全删除原语）

存储原子化轮开工前，执行会话就 §5 的内部不一致、A9–A11 的处置、以及安全删除原语要落在哪里提出判断点，
用户逐条裁定如下。**本次修订不改 Q1–Q9 中任何一条的方向**：它订正一处计数、把一处有意偏离批准并登记、
补齐一份清单、并新登记一个未决的残余。

| 条目 | 从什么 | 改成什么 | 依据 |
| --- | --- | --- | --- |
| **§5** 候选轮范围 | 开头句写「范围两件事」，而同一节末尾与 §6 拆轮表都写「三件」 | **外科式订正为「三件」**，并把绑定理由改写成「前两件同类，第三件是同一条排期链上的前置条件」；同时在原地写清这是订正而不是重写 | 用户裁定「只做外科式订正，不重写整段历史」。第三件（附件的安全删除）是第七次裁定并入的，当时只加了条目、没有改开头句 |
| **§3 · A9 / A10 / A11** | 处置列写「照搬」 | **保留历史事实与编号**，处置列标注**已被第十二次裁定取代**（原生改条件写 + 受影响行数校验）。编号**不退役、不复用**；Electron 侧的形态一字不改 | 用户裁定。A 系列表头写的是「Electron 的已知形态」，那部分仍然为真；变的是原生的处置，所以那属于 B 系列 |
| **§3 · B11** | （本次新产生） | 新增 **B11**：三处生命周期写在原生是条件写、在 Electron 是无条件写；连同 `remove` 的 `RETURNING` 取孤儿路径、`updateTaxInvoice` 的三个谓词、以及一处**无稳定错误码的残余**（走非 `BusinessDocumentError`，页面呈现既有的通用「保存失败」） | 用户**明确批准**这处有意偏离，并要求按 A/B 分工登记。B11 是本系列第一条方向为「原生更严」的条目 |
| **升级条款** 的「附件删除」触发器 | 未写例外 | 明写一条例外：**`BackupRestore.clearActiveSlot` 不是本条款所指的路径**——它是关闭 store 之后的**整槽恢复**，删的是整个目录、不与任何 check-then-act 判据同处一个窗口；条款要防的是**在线的、逐附件的**删除 | 用户裁定。不写清楚的话，下一个读到条款的人会合理地认为条款早就被触发过 |
| **§3** 泄漏与 D-6 接缝清单 | 文中散着「重选 / 移除 / 取消」与两处回传值，没有一份完整清单 | 补成**五处**：重选、移除、**取消**、保存时替换/清除、删单据；前三处只改草稿状态，后两处有回传值 | 用户裁定「补齐取消」。「取消」此前只出现在 Electron 那三条删除路径的括号里，没有进过本清单 |
| **安全删除原语的落点** | （本次新产生） | 新文件 `native/SoloLedger/Sources/SoloLedgerCore/Support/AttachmentDeletion.swift`。**不得**放进 `Documents/`，也**不得**塞进 `Migration/StagingIngest.swift` | 用户裁定。放进 `Documents/` 会推翻 A9–A11 登记依据之二里那个「`Documents/` 下文件系统调用数 **0**」的测量——那个 0 是承重的 |
| **删除的安全形态** | （本次新产生） | 批准 **descriptor-rooted、inode-bound** 删除这一安全偏离，并要求**复用**既有的 internal `DirectoryHandle` / `BoundRegularFile.open` / `unlinkIfStillBound`，不得复制弱化版 | 用户裁定。删除是不可逆的，按名字删会被父目录换名或同名替身重定向 |
| **附件目录的注入** | （本次新产生） | Core 原语**不得**自己调用 `AppPaths.nativeAttachmentsDirectory()`；目录由调用者传入 | 用户裁定。那个访问器会**创建**它命名的目录，从 Core 调用会让一次删除尝试凭空造出一个真实目录 |
| **`ProductCatalog.mapWriteFailure`** | 匹配字面 `"(code 19)"` | 改判**主结果码**（复用 `LedgerStore.isConstraintViolation`），`FOREIGN KEY` 判断**排在前面**；谓词提升到 internal/static 以便测试喂真错误，public API 净增 0 | 用户裁定，并写清实际矩阵：`products` PK **1555**、NOT NULL **1299**、CHECK **275**、真实 `ON DELETE RESTRICT` **1811**、UNIQUE index 对照 **2067**、主码均为 **19**。**1811 是 `SQLITE_CONSTRAINT_TRIGGER`**（RESTRICT 是一个 action），所以不得把所有 787 机械替换成 1811；NO ACTION 的 **787** 只作对照，本轮在一个一次性库里实测到它 |
| **文件删除的失败** | （本次新产生） | 维持 **best-effort 静默**：不新增错误 case、不新增六语键、不新增用户文案；清理失败**不得**把已经成功的数据库写入误报成 `.saveFailed` | 用户裁定。数据库那一半已经落盘了，把清理的失败报成保存失败是在说谎 |
| **竞态 E** | （本次新产生） | 登记为**存储原子化的残余**，**不入 B 系列、不新开 A 编号**；并成为 **D-6 接删除前的强制前置裁定**：两条候选路径（无 schema 协调 + 外部写入残余的处置 / schema 所有权记录）由用户择一 | 用户裁定。D 窗口（引用扫描↔`unlink`）本轮已用 `BEGIN IMMEDIATE` 关上并有双连接实测；E（文件删掉之后名字被**重新认领同一路径**）关不上，**不得用一条永远为绿的假测试声称它已关闭** |
| **App 接线** | （本次新产生） | 本轮**不接**：App 目标对 `AttachmentDeletion` 的命名数必须为 **0**，两个 `_ = try …` 丢弃点保持原样；只允许订正 `FilePanels.swift` 与 `AppModel.swift` 里错误声称「原子化轮接线」的**注释** | 用户裁定。那两处注释写的是「the seam the atomicity round connects」，而接线是 D-6 的 |

**复核补丁（同一次裁定，同一个 PR）**：开出后的对抗式复核抓到本次修订自己造成的六处内部不一致，一并
外科式订正，**不改任何口径方向**——①§5 第 1 件写死的谓词形状 `WHERE id = ? AND status = ?` 与实际实现
相反（实现钉的是规则）；②§5 第 2 件把 `products` 的外键扩展码写成 787，而 RESTRICT 实际报 **1811**；
③§5 第 3 件仍写「重选/移除/替换/删单据时**真的删掉**」，与本次同时写下的「原子化轮不接」「五处」正面
冲突；④§3「并发窗口三条」的依据 1 仍断言「原生在这三处是忠实照搬」；⑤B11 把 0 行的处置说成一律给稳定
错误码，漏了两处例外；⑥§8 的「无未裁定项」没有为竞态 E 留任何指针，D-6 只看 §8 会绕过强制前置。
另有两处措辞订正：「取消此前只出现在括号里」被本节上方八行推翻，改为「此前从未并排列成清单」；
「命名数为 0」补上「剥注释后」——本轮订正过的那两处注释确实提到了这个名字。

**复核补丁二（同一次裁定，同一个 PR）**：评审 bot 在 `AttachmentDeletion` 上提了一条 P2 —— 属实。
`unlinkIfStillBound` 是 `fstatat` 然后 `unlinkat(name)`，同 UID 的进程在这两个相邻系统调用之间换掉同名
条目，被删的就是替身。**没有自行处置**：Darwin 没有按 inode 删除的系统调用，而本次裁定又明确要求复用
`unlinkIfStillBound`、不得另写变体，所以形状不由实现轮定。按既定协议**登记并停下要裁定**，与竞态 E 并列
写进 §5，三个候选方向列在那里。代码零行为改动，只把原语文档里那句「`unlink` 只在名字仍指向同一 inode 时
发生」订正成准确的说法。

**复核补丁三（文档契约一致性，零行为改动）**：独立复核判定 head `0a84f82` 暂不通过 —— 主体代码与承重
测试通过，但文档在两处仍把 D-6 的前置写成**一处**，而原语的 `Outcome` 注释还在**过度声称**。按复核授权
只做这一轮文档补丁：

* **§6 的 D-6 行**改为两个**并列**强制前置：**a. 竞态 E**；**b. `fstatat`→`unlinkat` 相邻系统调用窗口**。
  两条各自独立，**答一条不解另一条**。
* **§8 的开工闸门**由「一处」订正为「两处」，各自指回 §5 对应的那一节。
* **§3 的 D-4 附件一节**同步：原写「还多一道前置：竞态 E」，改为两道并列。
* **§5 两节原样保留**，包括各自的三个候选方向 —— **本轮仍不替用户选**。
* **`AttachmentDeletion.Outcome` 的注释收窄**：`.deleted` 只表达 `unlinkat` 对**那个名字**返回成功、
  且事后名字不再指向绑定的 inode，**不再声称被删掉的必然是绑定的那个 inode**；`.movedUnderUs` 只表达
  **在身份校验那一刻**名字已不指向绑定 inode、因此本次没有继续 unlink，**不再声称所有相邻窗口里的替身
  都不会被删**。
* **测试**：`testAStandInSwappedInDuringTheWindowIsNotRemoved` 更名为
  `testAReplacementAlreadyInPlaceAtTheIdentityCheckIsNotRemoved`，注释收窄到「替换在身份校验**之前**
  完成」。**没有新增任何假装关闭相邻窗口的测试**；测试文件头把两条未关缺口并列写明。
* **`FilePanels.swift` / `AppModel.swift`** 的注释并列列出两道闸门，**代码字节仍然不变**（机器验证：
  剥注释后字节等同）。

除测试更名外，**Swift 源码的非注释行零改动**。

**本次修订不改代码之外的任何其它口径**：Q1–Q9、Q2 的四条细目、A5 / A6 / A8 / A12、B1–B10 一个字未动。
D-6 本身仍未开工。

### 2026-08-21 · 第十三次裁定（D-6 开工：两道并列前置各自的答案）

D-6 的两个强制前置——**竞态 E** 与 **`fstatat`→`unlinkat` 相邻系统调用窗口**——由用户**分别**裁定。
两条彼此独立，答一条不解另一条，所以下面是两条裁定而不是一条。**本次修订不改 Q1–Q9 中任何一条的方向**：
它把两个开工闸门从「待裁定」改成「已裁定，残余在效」，并订正一处计数。

| 条目 | 从什么 | 改成什么 | 依据 |
| --- | --- | --- | --- |
| **§5 · 竞态 E** | 「两条候选路径由用户择一」，未决 | **选定第 1 条：无 schema 协调。** 不加附件所有权表、墓碑列或任何 schema 结构；受支持的 App 写入者遵守四条约定（名字一次性、创建 no-overwrite、不缓存失去所有权的旧路径、删除只走 `AttachmentDeletion.deleteIfUnreferenced` 一个入口） | 用户裁定。**如实记录：这是两个选项中用户的选择，不是本文件的推荐项**，另一项（schema／所有权记录）原文仍列在该节 |
| **§5 · 竞态 E 的残余** | （随上一条产生） | **明确接受不协作外部写入者与陈旧写入者的残余**：它们仍可能在删除后重新认领旧名字。**不得写成「竞态 E 已关闭」** | 用户裁定。无 schema 协调这条路对不协作的写入者没有约束力，那部分残余按裁定留下来 |
| **§5 · 竞态 E 的升级条款** | （本次新产生） | **自动升级**：一旦引入受支持的多进程写入、云同步、监控文件夹、外部编辑器回写，或任何跨越删除缓存旧路径的正式入口，本裁定自动失效，必须重新停下来裁定 schema／所有权记录 | 用户裁定。与 §3 既有升级条款同形：触发时不需要重新裁定要不要修，只需要裁定怎么修 |
| **§5 · 相邻系统调用窗口** | 「三个候选方向 (a)/(b)/(c)，本轮不替用户选」，**待裁定** | **选定 (a)：接受并登记残余。** 保留现有 `unlinkIfStillBound`；不另写改名后删除的变体；不把附件目录描述成获得系统级独占；**明确认可**同 UID 进程在这两个相邻系统调用之间替换同名目录项时替身可能被删 | 用户裁定，四条依据：①Darwin 无 unlink-by-inode；②改名后删除只挪窗口并新增孤儿路径；③原生仍严格强于 Electron 的路径直接删；④支持范围内的单据写入由 App 主执行序列协调，**但不得声称防住敌对或不协作的同 UID 进程** |
| **过度保证禁令** | （本次新产生） | 本章任何地方不得出现：「替身永远不会被删除」「删除针对绑定 inode 原子完成」「相邻系统调用窗口已经关闭」「目录已被系统级独占」 | 用户裁定。四句话逐字写死，守门按 §5 两节的在效区段切片检查 |
| **§5 · 相邻窗口的升级条款** | （本次新产生） | **自动升级**：若发现受支持的 App 内部路径可并发替换同名目录项、附件目录改为共享／同步目录，或产品需要承诺抵抗同 UID 写入者，本裁定自动失效，必须**暂停删除接线** | 用户裁定 |
| **§5 · 相邻窗口的登记依据③** | 「本轮 App 侧零调用，实害当前不可达」 | **原文保留并原地标注已由 D-6 作废**：接线之后 App 侧调用数不再是 0。依据①②不受影响；第十三次裁定是**在知道 D-6 就要接线**的前提下接受这处残余的 | 实测。D-6 的 `AttachmentDeletion` App 侧调用点闭集见本轮 PR |
| **§6 · D-6 行** | 「两个并列强制前置，都必须先裁定」 | 改为「已由第十三次裁定分别回答」，两项残余说明**保留不删**，并写清 D-6 获准接线的条件（两条都已裁定 + 接线只调用既有原语、不改 syscall 形状、不新增 schema／错误 case／六语键／用户可见文案） | 用户裁定 |
| **§8 · 开工闸门** | 「两处开工闸门」，未决 | 改为「两处已经分别裁定的残余边界」，表格加两列（第十三次裁定的选择、被接受的残余与升级触发器）；末句由「必须先读那两节」改为「已被裁定不等于已被消除」 | 用户裁定 |
| **§8 · 竞态 E 的候选数** | 「三个候选方向列在那里」 | **外科式订正为「两个候选方向」** | 实测：§5 的竞态 E 一节自始只列了 **1. 无 schema 协调 / 2. schema 所有权记录** 两条编号项，而 §8 的指针写着三个（`origin/main:docs/BUSINESS_DOCUMENTS_SPEC.md` 第 530 行）。**不为凑数发明第三个候选** |
| **§3 · D-4 附件一节** | 「App 目标对该原语的代码命名数为 **0**」「两个 `_ = try …` 丢弃点一字未改」 | 原文保留（它记录的是存储原子化轮的验收线），**新增一条说明 D-6 已把五处全部接上**，并写清五处各自吃什么、为什么都在事务外、清理失败为什么不反报 | 实测。两句在 D-6 之后为假，删掉就不再是记录 |

### 2026-08-21 · 第十四次裁定（D-6 接线的可见性：`AttachmentDeletion` 提升为 public）

D-6 开工时实测出一处**§四.B 的停止条款第一条**所指的情形，执行会话停下来要裁定，用户裁定如下。

**实测事实。** `AttachmentDeletion` 在 `Sources/SoloLedgerCore/Support/AttachmentDeletion.swift` 里是
`enum AttachmentDeletion`（无 `public`），而 App 目标通过 `XCLocalSwiftPackageReference` 把 `SoloLedgerCore`
当**独立模块**链接（`Package.swift` 的注释原文；pbxproj 里没有任何 `SoloLedgerCore/` 源文件进 App 目标）。
把 `_ = AttachmentDeletion.self` 种进 `FilePanels.swift` 真跑一次 App 构建，得到
`error: cannot find 'AttachmentDeletion' in scope` + `** BUILD FAILED **`。**第十二次裁定要求 D-6 从 App
调用这个原语，而模块边界让它调不到**——这不是实现困难，是两条已裁定条款在本仓的模块结构下无法同时满足。

| 条目 | 从什么 | 改成什么 | 依据 |
| --- | --- | --- | --- |
| **可见性** | `AttachmentDeletion` internal，App 目标不可见 | **批准最小提升**：`public enum AttachmentDeletion` + 一个 public 入口点。零新行为、零新错误 case、零新六语键、零新用户可见文案、syscall 形状一字不改 | 用户裁定。**如实记录：这是三个选项中用户的选择**，另两项是「只做侧栏激活、五处接缝留到下一轮」与「全停等裁定」 |
| **入口点形状** | （本次新产生） | **public 入口点返回 `Void`，不返回 `Outcome`**；带 `hooks:` 的既有那支保持 internal，`Outcome` 保持 internal | 用户裁定。返回 `Void` 让「清理失败反报保存/删除失败」在**类型层面**不可能写出来，直接把第十二次裁定「清理失败不得把已经成功的数据库写入误报成 `.saveFailed`」变成结构性保证；代价是 App 侧只能用磁盘事实断言，`Outcome` 断言留在 Core 测试与 `@testable` 的 App 测试里 |
| **public 声明数** | 995 | **997**（`public enum AttachmentDeletion` + `public static func deleteIfUnreferenced(_:in:using:)`）。按 `grep -rhE '^\s*(public\|open)\b' --include='*.swift' Sources/SoloLedgerCore` 复算 | 用户裁定。**不走 `public extension LedgerStore` 那条路**：那样加出来的方法不匹配上面这条 grep，报出来的棘轮会不动而 public 面实际变大——那是在糊弄度量；而且 `DocumentStore.swift` 在 `Documents/` 下，会推翻 §3 承重测量「`Documents/` 下文件系统调用数 0」 |

**本次修订不改任何已裁定口径的方向**：Q1–Q9 与 Q2 的四条细目、§3 的 A 与 B 两系列、§5 两处残余的处置、
§6 拆轮表的其余各行一个字未动。
