...
3. **附件的安全删除**：把 D-4 登记的那条泄漏收掉，
   ...
   > 两个 `_ = try …` 丢弃点一字未改。**接线仍然是 D-6 的**，且现在还多一道前置：竞态 E 必须先裁定。
...
| **D-6 激活** | 侧栏可达 + 接上附件删除接缝（**前置：竞态 E 必须先裁定，见 §5**）+ 收尾 | 侧栏入口可达；**`SidebarSection`（`AppModel.swift`）新增一例、`RootView` 加分支、`InventoryView` 的「不是本页的五个分区」注释与 `AppModel` 的「六个分区之一」注释同轮改**（第七次裁定自 D-4 移入；实测届时另有 8 处 `SidebarSection.allCases` 有序断言与 `DocumentCopyTests` 的 `SidebarSectionProbe` 两条要同批翻转，开工前须重跑扫描）；`LegacyLedgerProbe` 的 `otherRecordTables` 移除 `business_documents` 与 `business_document_items` 两项；`legacy.other.message` 六语改写（现文说「本 App 目前只显示流水」，激活后变假）；`docs/SWIFTUI_FEATURE_GAP.md` §3 该行改状态 | +10~20 | `products` 已有同形先例，其理由写在 `AppModel.swift` 对 `otherRecordTables` 的注释里 |
...
**一处开工闸门（第十二次裁定新增）：竞态 E。** 上面那个「无」说的是**测绘九问**——本章的口径没有留白，Q1–Q9 全部已裁定，这一点未变。但存储原子化轮新产生了一个**必须先裁定才能继续**的技术缺口：附件文件删掉之后那个相对名会被重新认领。它不是九问里的哪一问，所以不在上表；它写在 **§5 的「竞态 E」一节**，并且是 **D-6 接删除前的强制前置裁定**。**D-6 开工前必须先读那一节**，不能只看本节的「无」就认为没有阻塞。
