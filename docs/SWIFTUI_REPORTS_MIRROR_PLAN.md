...
4. ❌ **"仅写迁移说明"不予批准**——前提被推翻,见 §9.1。待你在两个方案间拍板。

**硬约束(升格)**:"未配置"的判定**必须**基于设置行的缺失,**不得**基于计算出的值,并须配测试。见 §6.2。
...
- **设置行缺失**的账本,含**中国 NaN 路径**(缺附加税率时 cn.js 产出 NaN)
...
> **待你拍板(§10 问题 4)**:方案 A(失败即拒:非中国 + 缺行 → 不计算、返回 null,由 UI 显示"未配置",与原生侧口径对称,不发明任何数字)对比方案 B(按 `accountingProfiles` 预设做分制度兜底,两行改动 + 守护脚本,但等于**替用户选了一个税率**)。两者都改受保护文件,都需你明确批准;中国行为在两案中均保持不变。
...
4. ⏳ **待拍板**:跨制度兜底走方案 A 还是方案 B(见 §9.1)。在此之前 R6/R7 不动;R0–R5 不受影响,可以先行。
