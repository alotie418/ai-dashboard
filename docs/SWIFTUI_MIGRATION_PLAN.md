...
> 状态：Phase 1 技术原型规划（草案，2026-07-15）；**当前实现进度见 §0（更新至 2026-07-20，已落地至 2B-3 / C12x-A2）**
> 目标分支：`feat/swiftui-native-rewrite`（已合入 `main`）
> 原型目录：`native/SoloLedger`
> 编写日期：2026-07-15；最近状态更新：2026-07-20
...
## 0. 当前实现状态（2026-07-20 更新）

> 本节是权威的**当前状态**记录，取代下文 Phase-1 原型期的部分约束（尤见 §2.4 / §5.3 / §8）。下文自 §1 起为原始规划，保留作历史脉络，不重写。

### 0.1 已落地进度：2B-3 / C12x-A2

原生 SwiftUI 版已**不再是只读 dev 沙箱原型**。截至 2026-07-20，`native/SoloLedger` 已实现并合入 `main`：

- **生产启动链（C12a / C12b-1/2/3）**：`AppModel.boot()` 走 `MigrationCoordinator` 的 probe→confirm→open 裁决链（off-main 探测、main-actor 同步 confirm→open、单飞 + generation 防陈旧）。协调器**自身从不构造 store**，开库由授权驱动。
- **恢复 UI（C12b-3）**：`MigrationPresenter` + `RootView` 呈现 running / 可重试 / 终态 / 确认 / 导入选择 / 残留 banner 等 C12 状态，与旧的 `DatabaseUpgrade` 阻塞恢复界面互斥。
- **Hardened open（C12x-A1 / A2）**：两条 active-store 开库路径都已加固并接线——
  - existing：`openActiveExistingHardened`（NOFOLLOW 打开 + HAS_MOVED + parent(dev,ino)+leaf 指纹 vs 证据，单次开库，确定性关闭后再 adopt）；
  - createFresh：`createFreshReservedHardened`（`openat(O_CREAT|O_EXCL|O_NOFOLLOW)` 独占 reservation → 单次 NOFOLLOW 开库 → HAS_MOVED + 当前路径 parent 身份 + leaf 指纹 vs 已保留 inode → SQL 前显式关 reservation → adopt）；
  - 接线：`AppModel.openStoreForPlan` 把 `.createFresh` → `createFreshReservedHardened`、`.existing(evidence)` → `openActiveExistingHardened`，有生产接线守卫测试。
- **自动迁移来源**：生产唯一构造的 `MigrationSource` 是 `.masContainer`（`AppModel.swift`），解析到 **Electron-MAS 容器**内的 `sololedger.db`（与原生版同 Bundle ID `com.alotie418.sololedger`、同沙箱容器族）。

因此 §2.4 / §5.3 中「原型只读写 dev 沙箱、**绝不打开生产账本**」是 **Phase-1 原型期约束，已被上述生产启动链取代**：发布形态的原生 App 现在会在协调器授权下，对**真实活动库**做加固开库、迁移与 fail-closed 恢复（失败从不用空库遮盖）。
...
- **Electron DMG（非沙箱）版**数据在容器之外的 `~/Library/Application Support/SoloLedger/sololedger.db`，沙箱化的 MAS 原生 App **在无用户授权时够不到**。
- 结论：**DMG 用户的账本在原生 MAS 版里当前无法迁移**，只会被 createFresh 建的空库"绕过"。这是**发布前 P0**（见 §8 风险登记与 `SWIFTUI_FEATURE_GAP.md` 阻塞清单）。

### 0.3 P0 的处置约束（N7，先设计、不实现）

弥合该缺口的改动（暂称 N7）**必须先做只读设计分析、经用户确认后才能实现**，当前阶段**不落地代码**。已确定的约束：

- **不能**简单在现有 Onboarding 里加个按钮：启动流程可能已经 `createFresh` 建了活动库，之后再导入会与已存在的 active store 冲突。**来源选择（迁移旧版数据 / 创建新账本）必须发生在 createFresh 之前。**
- 采用 **single-grant-window**（一次授权窗口）：**不持久化 security-scoped bookmark、不新增 `com.apple.security.files.bookmarks.app-scope` entitlement**（现有 `user-selected.read-write` 足够一次性导入；新 entitlement 会扩大 MAS 审查面）。
- 中断语义须明确：staging 发布**前**中断 → 重新选目录；发布**后** → 从内部 staging 恢复。
- 设计须覆盖：启动状态机、单飞/取消、错误 UI、目录预检、MAS 沙箱权限，以及**防止覆盖已有 active store 的守卫**。

### 0.4 仍不在近期范围

- **Phase 2 报表与会计公式**（损益 / VAT-GST / 现金流 / COGS / `_expenseSplit` / 库存成本等）仍**排除在近期范围外**：须逐字**镜像** `electron/reports/*` 并经会计师/用户确认，不重造（CLAUDE.md 硬门禁）。
- 其余 Release 前阻塞项（附件迁移、legacy `sales`/`purchases` 二次迁移、旧进程检测硬化、真实 Release 沙箱端到端验证、6 语言 parity、MAS 打包）见 `SWIFTUI_FEATURE_GAP.md`。
...
> ⚠️ **状态更新（见 §0）**：本小节「只读写 dev 沙箱、绝不打开生产账本」为 **Phase-1 原型期**约束，自 2B / C12 起已被生产启动链取代——生产 App 现在会经协调器授权对**真实活动库**加固开库/迁移。以下保留作历史脉络。
...
- 两种情形都与生产文件物理隔离。原型**绝不**打开生产文件——即便 schema 完全兼容，实验代码也不碰用户唯一账本。测试 / self-test 只用临时目录。（**状态更新，见 §0**：此为 Phase-1 原型期约束；生产启动链自 2B / C12 起会在协调器授权下对真实活动库加固开库/迁移。）
...
- **只读**检视 / 导入已有 SoloLedger DB（先只读，绝不写生产文件）  ← **状态更新（§0）**：生产启动链（2B / C12）已能加固开库 / 迁移真实活动库；剩余缺口为 DMG 目录的用户授权入口（发布前 P0，N7 先设计经确认）
...
| 原生 **Release**（同 Bundle ID → 同容器）活动库 | `…/Containers/com.alotie418.sololedger/Data/Library/Application Support/**SoloLedgerNative**/sololedger.db` |
...
- 数据文件夹名按构建配置区分（`AppPaths.nativeDataFolderName`，`#if DEBUG`）：**Release = `SoloLedgerNative`**（生产不带 "Preview" 字样），**Debug = `SoloLedgerNativePreview`**。
- Release 与 Electron **共享容器**，故能发现 `SoloLedger/sololedger.db`（升级源）；Debug 在**独立 `.dev` 容器**，其中无生产数据 → 发现返回 `noLegacyData`，**永不触碰生产库**，且**不申请**任何访问生产容器的额外权限。发现路径是**容器相对**的（同一 Application Support），由 `PathIsolationTests` 断言，保证 Debug 永远只在 `.dev` 容器内解析。
- 原生活动库放在**独立目录**；Electron 原始 `SoloLedger/sololedger.db` 只被**文件读取**，**永不被 SQLite 打开写入、永不修改 / 删除**。

**升级流程（`DatabaseUpgrade`）**：
1. **发现**（源在？活动库不在？）
2. **稳定快照**：仅文件拷贝 `.db` + `-wal`（**不拷 `-shm`**——它是可从 `-wal` 重建的 WAL-index 共享内存缓存，复制过期 `-shm` 反而会损坏读取；SQLite 打开副本时自动重建）到 staging。**复制前后对源做指纹（size + mtime）比对**；若源在复制期间变化（旧 App 仍在写）→ 重试，仍不稳定则以 `sourceBusy` 阻止并提示"退出旧版 App 后重试"。**只有指纹稳定后，这份拷贝才算 point-in-time 快照**——不把普通文件复制直接当作一致快照。
3. 在**副本**上 `wal_checkpoint(TRUNCATE)` 折叠 WAL + 规整单文件 → `quick_check`
4. **版本闸**（`>23` 视为未知 → 拒绝）
5. **SQLite Backup API** 从规整后的副本生成保留备份并校验
6. 在副本上跑 `SchemaMigrator` + `integrity_check`
7. **原子切换**（`replaceItemAt` / `moveItem`）
8. 任一步失败**回滚**（清副本，保留原库 + 备份）。原始库**永不删除**。

**迁移失败 = 阻塞恢复（P0）**：`prepareActiveDatabase()` 返回 `.blockedMigrationFailed`（旧库存在但升级失败）时，`AppModel.boot` **绝不创建活动库**（不用空账本遮盖失败），进入阻塞恢复界面：**重试迁移 / 查看错误 / 从备份·导出文件恢复 / 二次确认后创建空白账本**。因失败从不建活动库，下次启动会重新发现并重试（`testFailedUpgradeLeavesNoActiveDBThenRetrySucceeds`）。

**测试覆盖（54 项全绿）**：正常升级、损坏库、未知版本、备份失败、迁移中断 + 幂等恢复、失败不建库 + 重试成功、**经 `run()` 全链路的未 checkpoint WAL 行落到活动库与备份**、过期 `-shm` 不复用、源库变化检测（`sourceBusy` 阻止 / 稳定后恢复）、路径隔离、无数据 / 已升级。fixture 由**真实 `electron/db` 迁移代码**（`ELECTRON_RUN_AS_NODE=1` 跑仓库 electron 二进制，绕开 better-sqlite3 的 Electron-ABI 限制）生成，并用 **sqlite3 CLI 独立交叉验证**。

**仍未覆盖**：旧进程检测仅靠**文件指纹变化**（沙箱内无法直接枚举其他进程/取文件锁）；多货币汇总按 Electron 语义做**朴素数值求和**（不按币种分组）；attachments 附件文件迁移、legacy `sales/purchases` → `transactions` 的二次数据迁移、`ai_providers` / `ecommerce_connections` 的加密列（safeStorage 密文，跨应用不可移植）均未纳入本阶段；未在真实生产库上端到端执行（按约束只用 fixture）。
