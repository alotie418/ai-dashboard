...
> **状态更新 2（PR-C 后）**：**签名/公证已完成**（#355 + PR-C 真机执行成功，见 §2 表与 [`RELEASE.md`](RELEASE.md) §9）——**对外分发已无硬 blocker**；版本 **1.0.0-rc.1**，正式 1.0.0 门槛 = §5 第 4/5/6 条人工验收。
...
3. **版本策略**：✅ 版本已定 **1.0.0-rc.1**、CHANGELOG 已填（发布收尾 PR）；「检查更新」入口**明确暂缓**——发布渠道未最终确认，不引入新 UI/逻辑。
4. **xlsx 真实 `.xlsx` / `.xls` 导入人工冒烟**（#352 解析器 0.18.5→0.20.3，见 §6 清单）——未完成。
5. **WooCommerce / Shopify 真实店铺人工 QA**（照 [`ECOMMERCE_WOO_REAL_STORE_QA.md`](ECOMMERCE_WOO_REAL_STORE_QA.md)）——未完成。
6. **干净机断网 Gatekeeper 冒烟 + safeStorage 重录 QA**（1.0.0 正式版门槛，见 §6 清单与 [`RELEASE.md`](RELEASE.md) §5）——未完成。
...
- [x] Gatekeeper 打开流程：~~未签名右键「打开」~~ 已由签名+公证取代——本机安装后 `spctl accepted (Notarized Developer ID)`、双击直接打开（2026-07-07）；**干净机 + 断网验证仍待跑（见上一项与下一项）**
