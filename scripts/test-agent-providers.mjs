...
async function testAnthropic() {
  ...
  const tr = anthropic.toToolResultMsg(s1.calls[0], { rows: [1, 2] });
  check('anthropic toToolResultMsg shape', tr.role === 'user' && tr.content[0].type === 'tool_result' && tr.content[0].tool_use_id === 'tu_1' && tr.content[0].content === JSON.stringify({ rows: [1, 2] }));
...
async function testOpenAI() {
  ...
  const tr = openai.toToolResultMsg(s1.calls[0], { rows: [1, 2] });
  check('openai toToolResultMsg shape', tr.type === 'function_call_output' && tr.call_id === 'call_1' && tr.output === JSON.stringify({ rows: [1, 2] }));
