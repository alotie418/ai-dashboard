...
| 🛍️ 电商订单导入(**Beta**) | Shopify / WooCommerce 订单拉取 → 暂存 → 预览 → 提交入账辅助(写入销售);孤儿单解锁重提;隔离演示模式。**记账辅助,非税务合规/ERP;Beta:未经真实店铺实证**,详见下文 |
...
> **Beta 状态(2026-07-08 决策)**:本模块**本地自动化覆盖充分**(EC1–EC27,含提交幂等 / 整单对账守卫 / 隔离演示模式),但**未经真实店铺实证**——真实店铺联调(TLS / 权限语义 / HPOS / 限流 / 量级性能等)为**发布后验证项**,尚未完成真店 QA。待验证清单见 [`docs/ECOMMERCE_MVP_STATUS.md`](docs/ECOMMERCE_MVP_STATUS.md) §8。
