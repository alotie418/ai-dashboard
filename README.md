...
| 💰 财务报表 | 经营损益概览(利润表 / Schedule C 口径);资产负债与现金流概览开发中 |
...
| **DeepSeek 深度求索** | `deepseek-v4-pro` | — | OpenAI 兼容 Chat Completions |
| **Qwen 通义千问** | `qwen-plus` | ✅ | OpenAI 兼容 Chat Completions |
| **Kimi (Moonshot)** | `kimi-k2.6` | ✅ | OpenAI 兼容 Chat Completions |
...
- **本地优先**:全部业务数据存于本地 SQLite(`better-sqlite3`,位于应用的 `userData` 目录),无云端账户、无服务器存储。
- **API Key 加密**:Key 经 Electron `safeStorage` 加密后写入本地数据库,渲染端拿不到明文;删除 Key 后对应服务商立即停用,已录入的业务数据保留。
- **请求直连**:AI 请求由你的 Mac **直接发往你选择的服务商**,SoloLedger 不参与任何转发。
- **备份与恢复**:支持导出/恢复整库快照(校验 + 覆盖前自动备份 + 原子替换)。
  > 注意:当前备份只含数据库,**不含发票附件文件**;迁移到新机器或恢复旧备份时,附件引用可能失效。自动备份与附件打包属路线图项。

---

## 技术栈

- **桌面**:Electron 33(主进程 `electron/`,IPC 白名单 + `contextIsolation`)
