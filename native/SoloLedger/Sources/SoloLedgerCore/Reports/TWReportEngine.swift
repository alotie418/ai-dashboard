...
/// 台灣報表引擎 —— batch-1 的損益核心，加上流轉稅與月度區塊。Mirror of `electron/reports/tw.js`.
///
...
enum TWReportEngine {
    ...
    static func batchOne(_ ctx: ReportContext) -> BatchOneIncomeStatementWithOperatingProfit {
        ...
        let r = ReportMath.round2OrZero                       // tw.js generate — has `|| 0`

        // tw.js totalIncomeNet
        ...
        // tw.js rateMissing — the estimate layer. `refusal` is the ONLY path to a refusal:
        ...
        // tw.js tax — clamped at 0, so a loss period pays no income tax and netProfit
        // equals operatingProfit. Rounded ONCE here…
        let taxPayable = rate.map { r(ReportMath.max(0, operatingProfit) * ($0 / 100)) }
        // …and NOT rounded again at the emit (tw.js incomeTax passes `taxPayable`
        // straight through), unlike China which rounds it twice.
        let incomeTax = refusal ?? .computed(taxPayable ?? 0)
        // tw.js netProfit — the UNROUNDED operating profit minus the ROUNDED tax.
        ...
        return BatchOneIncomeStatementWithOperatingProfit(
            ...
            // emitted above: tw.js tax multiplies the local. Taking the struct's
...
extension TWReportEngine {
    ...
    /// `tw.js taxInclusiveSummary` — the tax-inclusive summary.
    ///
    /// TAX-INCLUSIVE sums, not the net ones the P&L uses. This engine's rounder
    /// HAS the `|| 0` guard (`tw.js generate`), unlike China's — so a NaN becomes 0 here
    /// and `null` there.
    ...
    static func businessTax(_ ctx: ReportContext) -> TWBusinessTax {
        ...
        // tw.js businessTaxPayable — the local name is `businessTaxPayable`.
