...
async function list() {
  const db = getDb();
  const rows = db.prepare('SELECT * FROM purchases ORDER BY date DESC').all();
  // P4b-2 list expand: attach line items so the UI can render one row per item. One extra
  // query, grouped in JS by purchase_id. Legacy single-item records have no purchase_items and
  // get no items field (so they render as a single row). Read-only — additive field, no calc
  // touched (inventory/AP/reports run their own queries; tons stays 0 per P2/P3).
  const items = db.prepare(
    `SELECT id, purchase_id, product_id, description, unit_snapshot, quantity, unit_price,
            amount_net, tax_rate, tax_amount, amount_gross, line_no
       FROM purchase_items ORDER BY line_no, id`
  ).all();
  if (items.length === 0) return rows;
  const byParent = {};
  for (const it of items) (byParent[it.purchase_id] ||= []).push(it);
